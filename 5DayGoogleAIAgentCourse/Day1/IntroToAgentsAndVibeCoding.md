...
⋅⋅* Conductor mode: hands-on, real-time direction → highly interactive, keystroke to keystroke, immediate sub-second feedback (helpful for debugging or navigating through codebase that human intuition is still required for)
⋅⋅* Orchestrator mode: asynchronous, high-level delegation → assign broad goals to background terminal agents (let the agent work independently in a sandbox and then review the pull request few hours later)
